...
const traits = ref(["fate's favored", "reactionary"]);
    ...
    class: [
      {
        ...
        spells: {
          ...
          "3rd": {
            ...
            prepared: ["Haste", "Glibness", "Major Image", "Charm monster"],
          },
          "2nd": {
...
const level = computed(() =>
  flafaffiel.value.introduction.class.reduce(
    (accumulator, cur) =>
      cur.gestalt ?? false
...
const skills = computed(() => {
  ...
  const tempClassSkills = ref(
    flafaffiel.value.introduction.class[0].classSkills
  );
...
const savingThrows = computed(() => {
  ...
  saveKeys.forEach((save) => {
    ...
    totalSaves[save] +=
      abilityMods.value[flafaffiel.value.defense.saveAbilityScore[save]];
...
const specialAbilities = ref([
  {
    name: "Arcane Pool",
    type: "Su",
    header: `Arcane Pool (Su) ${
      abilityMods.value.charisma +
      Math.floor(level.value / 2) +
      Math.floor(level.value / 4)
    }/day`,
    description: [
